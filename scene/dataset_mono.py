...
class MultiViewDataset(Dataset):
    def __init__(self, dataset_parms, device=torch.device('cuda:0')):
        ...
        # 基础路径（指向包含所有视角的父文件夹）
        base_data_folder = join(dataset_parms.source_path, 'train')
        # 加载共享的smpl_data（关键修改）
        if dataset_parms.train_stage == 1 or dataset_parms.train_stage == 3:
        ...
        # 将smpl_data提升到多视角数据集（按用户要求）[0]没有特殊意义 因为每个视角的pose是相同的
...
class MonoDataset_train(Dataset):
    @torch.no_grad()
    def __init__(self, dataset_parms,data_folder=None, smpl_data=None,
        ...
        #print('mydatafolder',self.data_folder)
        # self.data_folder = join(dataset_parms.source_path, 'train')
        self.device = device
    ...
    def getitem(self, index, ignore_list):
        ...
        if self.dataset_parms.train_stage == 2 or self.dataset_parms.train_stage == 1 or self.dataset_parms.train_stage == 3:
            ...
            inp_posmap_path = self.data_folder_inp + '/inp_map/' +'inp_posemap_%s_%s.npz'% (str(self.dataset_parms.inp_posmap_size), str(pose_idx).zfill(8))
        ...
        data_item = dict()
        if self.dataset_parms.train_stage == 2 or self.dataset_parms.train_stage == 1 or self.dataset_parms.train_stage == 3:
            #print(inp_posmap.shape,'dataset')
...
class MonoDataset_novel_pose(Dataset):
    ...
    @torch.no_grad()
    def getitem(self, index, ignore_list):
        #print(self.dataset_parms.train_stage,'self.dataset_parms.train_stage')
        pose_idx  =  index
        if self.dataset_parms.train_stage == 2 or self.dataset_parms.train_stage == 1 or self.dataset_parms.train_stage == 3:
            inp_posmap_path = self.data_folder + '/inp_map/' +'inp_posemap_%s_%s.npz'% (str(self.dataset_parms.inp_posmap_size), str(pose_idx).zfill(8))
            inp_posmap = np.load(inp_posmap_path)['posmap' + str(self.dataset_parms.inp_posmap_size)]
