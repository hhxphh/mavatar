...
#        self.smpl_data = self.train_dataset.smpl_data
    ...
    # def compute_laplacian(self,posmap, iteration=0):
    #     # 创建扩展掩码
    ...
    # def twist(self, posmap1, posmap2, iteration=0):
    #     n = 128
    #     laplacian1, mask1 = self.compute_laplacian(posmap1, n)
    #     laplacian2, mask2 = self.compute_laplacian(posmap2, n)
    #     common_mask = mask1 & mask2
    #     diff_p = torch.abs(laplacian1 - laplacian2) * common_mask[..., None]  # 自动应用mask
    #     return diff_p, common_mask  # 返回共同有效区域掩码

    # def diff_p_f(self, posmap1, posmap2, feature1, feature2, iteration=0):
    #     diff_p, common_mask = self.twist(posmap1, posmap2)  # 获取diff_p和共同mask
    ...
    #     return diff_loss*0.05#0.1 L1 0.0017 SSIM 0.0023 diff -0.0967
    
    def twist(self, batch_data1, batch_data2, iteration=0):
        n = 128
    ...
    def train_stage(self, batch_data, iteration,stage,batchname=1):
        ...
        pred_scales = pred_scales.permute([0,2,1])* 1e-1
        ...
        if stage == 3:
            geo_loss = torch.mean(self.geo_feature**2)
            pose_loss = torch.mean(pose_featmap ** 2)
            return torch.stack(rendered_images, dim=0), full_pred, offset_loss, geo_loss, pose_loss, gs_loss,scale_loss,pose_featmap[0].permute(1,2,0)
        return
    ...
    def render_free_stage(self, batch_data, iteration,stage,idx):
        ...
        for batch_index in range(self.batch_size):
            ...
            if self.model_parms.dimension==2:
                ...
                return torch.stack(rendered_images, dim=0),torch.stack(normals_images, dim=0),torch.stack(depth_normals_images, dim=0), full_pred
