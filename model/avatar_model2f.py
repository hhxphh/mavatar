...
from model.network2f import POP_no_unet
...
#        self.smpl_data = self.train_dataset.smpl_data
    ...
    def load(self, epoch, test=False):
        ...
        if self.optimizer is not None:
            # 尝试匹配参数组
            saved_optim_state = saved_model_state["optimizer"]
            try:
                # 更详细的加载过程
                print("\n=== 尝试加载优化器状态 ===")
                ...
                for group in self.optimizer.param_groups:
                    current_param_ids.update(id(p) for p in group["params"])
                    
                saved_param_ids = set(saved_optim_state["state"].keys())
                
                print("\n=== 参数匹配诊断 ===")
                print(f"当前模型参数ID数量: {len(current_param_ids)}")
                print(f"保存的优化器状态参数ID数量: {len(saved_param_ids)}")
                
                # 查找丢失的参数
                missing_in_current = saved_param_ids - current_param_ids
                missing_in_saved = current_param_ids - saved_param_ids
                
                if missing_in_current:
                    print(f"保存的优化器中有 {len(missing_in_current)} 个参数在当前模型中不存在:")
                    for pid in list(missing_in_current)[:5]:  # 最多显示5个
                        print(f"  - ID {pid}")
                
                if missing_in_saved:
                    print(f"当前模型中有 {len(missing_in_saved)} 个参数在保存的优化器中不存在:")
                    for pid in list(missing_in_saved)[:5]:  # 最多显示5个
                        print(f"  - ID {pid}")
                
                print("\n建议:")
                if missing_in_current:
                    print("可能原因: 模型结构已更改(删除了参数层)")
                    print("解决方案: 使用较新的检查点或重置优化器状态")
                elif missing_in_saved:
                    print("可能原因: 模型结构已更改(添加了新参数层)")
                    print("解决方案: 忽略缺失参数的警告或使用部分加载")
                else:
                    print("可能原因: 参数组结构不匹配但参数一致")
                    print("解决方案: 尝试只加载参数状态而不加载组配置")
    ...
    def printzero(self, data,name):
        # 计算非零元素的数量
        nonzero_count = torch.count_nonzero(data)
        # 计算总的元素数量
        total_elements = data.numel()
        # 计算非零元素占比
        nonzero_percentage = (nonzero_count.float() / total_elements) * 100
        print(f"非零元素占比: {nonzero_percentage.item():.2f}%",name)
    def save_tensor_as_image(self, tensor, file_path):
        ...
        # 处理单通道和三通道的不同情况
        if tensor.size(2) not in (1, 3):
            raise ValueError(f"通道数应为 1 或 3，但实际为 {tensor.size(2)}")
        ...
        print(f"原始数值范围: {min_val:.6f} - {max_val:.6f}")
    ...
    def train_stage(self, batch_data, iteration,stage,batchname=1):
        ...
        normal_lpls_loss=torch.tensor(0,device='cuda:0')
        ...
        if stage == 3:
            geo_loss = torch.mean(self.geo_feature**2)
            pose_loss = torch.mean(pose_featmap ** 2)
            #self.printzero(rend_normal.squeeze(0).permute(1, 2, 0),'rend_normal.squeeze(0).permute(1, 2, 0)')
            normal_lpls_loss=self.twist(batch_data["normal"].squeeze(0),rend_normal.squeeze(0).permute(1, 2, 0),use_cal=True)
            #normal_lpls_loss=torch.mean(rend_normal)
            #print(normal_lpls_loss,'normal_lpls_loss')
