...
class ShapeDecoder(nn.Module):
    ...
    '''
    def __init__(self, in_size, d=3,hsize = 256, actv_fn='softplus'):#d=1
        self.hsize = hsize
        ...
        self.conv8N = torch.nn.Conv1d(self.hsize, d, 1)

        # Rotations branch
        self.conv6R = torch.nn.Conv1d(self.hsize, self.hsize, 1)
        self.conv7R = torch.nn.Conv1d(self.hsize, self.hsize, 1)
        self.conv8R = torch.nn.Conv1d(self.hsize, 4, 1)  # 输出四元数（4通道）

        # # Opacity branch
        # self.conv6O = torch.nn.Conv1d(self.hsize, self.hsize, 1)
        # self.conv7O = torch.nn.Conv1d(self.hsize, self.hsize, 1)
        # self.conv8O = torch.nn.Conv1d(self.hsize, 1, 1)  # 输出不透明度（1通道）
        ...
        # Batch normalization for new branches
        self.bn6R = torch.nn.BatchNorm1d(self.hsize)
        self.bn7R = torch.nn.BatchNorm1d(self.hsize)
        # self.bn6O = torch.nn.BatchNorm1d(self.hsize)
        # self.bn7O = torch.nn.BatchNorm1d(self.hsize)
    ...
    def forward(self, x):
        ...
        x6 = self.actv_fn(self.bn6(self.conv6(x5)))
        x7 = self.actv_fn(self.bn7(self.conv7(x6)))
        x8 = self.conv8(x7)  # Position output: (batch_size, 3, num_points)

        # Scales branch
        xN6 = self.actv_fn(self.bn6N(self.conv6N(x5)))
        xN7 = self.actv_fn(self.bn7N(self.conv7N(xN6)))
        xN8 = self.conv8N(xN7)  # Scales output: (batch_size, 1, num_points)
        #scales = self.sigmoid(xN8)  # Scales range [0, 1]
        scales = self.scaling_activation(xN8)
        # SH coefficients branch
        xSH6 = self.actv_fn(self.bn6SH(self.conv6SH(x5)))
        xSH7 = self.actv_fn(self.bn7SH(self.conv7SH(xSH6)))
        xSH8 = self.conv8SH(xSH7)  # SH coefficients output: (batch_size, 3, num_points)
        shs = self.sigmoid(xSH8)  # SH coefficients range [0, 1]

        # Rotations branch
        xR6 = self.actv_fn(self.bn6R(self.conv6R(x5)))
        xR7 = self.actv_fn(self.bn7R(self.conv7R(xR6)))
        xR8 = self.conv8R(xR7)  # Rotations output: (batch_size, 4, num_points)
        #rotations = self.tan(xR8)  # Rotations range [-1, 1] (for quaternion)
        rotations = self.rotation_activation(xR8)
        # Opacity branch
        # xO6 = self.actv_fn(self.bn6O(self.conv6O(x5)))
        # xO7 = self.actv_fn(self.bn7O(self.conv7O(xO6)))
        # xO8 = self.conv8O(xO7)  # Opacity output: (batch_size, 1, num_points)
        # opacitys = self.sigmoid(xO8)  # Opacity range [0, 1]
        opacitys = None

        return x8, scales, shs, rotations, opacitys
    # def forward(self, x):
    #     x1 = self.actv_fn(self.bn1(self.conv1(x)))
    #     x2 = self.actv_fn(self.bn2(self.conv2(x1)))
    #     x3 = self.actv_fn(self.bn3(self.conv3(x2)))
    ...
    #     xR6 = self.actv_fn(self.bn6R(self.conv6R(x5)))
    #     xR7 = self.actv_fn(self.bn7R(self.conv7R(xR6)))
    #     xR8 = self.conv8R(xR7)  # (batch_size, 4, num_points)
    #     rotations = self.tan(xR8)  # 使用 tanh 将四元数限制在 [-1, 1]
    #     #rotations = self.rotation_activation(xR8)

    #     # Opacity prediction
    #     xO6 = self.actv_fn(self.bn6O(self.conv6O(x5)))
    #     xO7 = self.actv_fn(self.bn7O(self.conv7O(xO6)))
    #     xO8 = self.conv8O(xO7)  # (batch_size, 1, num_points)
    #     opacitys = self.opacity_activation(xO8)  # (batch_size, 1, num_points)
